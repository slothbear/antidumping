...
/*                           MACROS PROGRAM                         */
/*                                                                  */
/*                 LAST PROGRAM UPDATE – JULY 15, 2015              */
/*                                                                  */
/********************************************************************/
/*                              GENERAL MACROS                      */
/*------------------------------------------------------------------*/
...
**************************************************************************************;
** G-2: CREATE FIXED TITLES AND FOOTNOTES ON LINES 1 AND 2 FOR EACH PRINT AND 		**;
**		PROGRAM-SPECIFIC MACROS FOR TITLES 3 AND HIGHER								**;
**************************************************************************************;
...
%MACRO G2_TITLE_SETUP;
	...
	%IF %UPCASE(&SALESDB) = CMSALES %THEN
	%DO;
		%LET PROGRAM = COMPARISON MARKET;
		%LET SALES_DB = COMPARISON MARKET;
	%END;
	...
	%END;
	%ELSE %IF %UPCASE(&CASE_TYPE) = INV %THEN
	%DO;
...
%MACRO G3_COST_TIME_MVARS;
	...
	%GLOBAL CM_TIME_PERIOD NAF_TIME_PERIOD COST_TIME_PERIOD US_TIME_PERIOD US_TIME AND_TIME
			EQUAL_TIME OR_TIME FIRST_TIME COP_TIME_OUT COST_PERIODS TIME_ANNUAL ANNUAL_COST;
	...
	%DO;
		...
		%LET US_TIME = US_TIME_PERIOD;  /* For Level of Trade Calculation in CM program */

		%IF %UPCASE(&USE_COSTDATA) = YES %THEN
		%DO;
			...
			%IF %UPCASE(&SALESDB) = USSALES %THEN
			%DO;
				%LET CM_TIME_PERIOD = CM_TIME_PERIOD;
			%END;
...
**************************************************************************************;
** G4: CREATE LEVEL OF TRADE VARIABLE FOR PROGRAMMING USE BASED ON INFORMATION IN 	**;
**	   MACRO VARIABLE 'LET CMLOT = <???>'											**;
**************************************************************************************;
...
%MACRO G4_LOT(LOT_REP,LOT_PROG);
	...
	%DO;
		&LOT_PROG = &LOT_REP;
	%END;
	%ELSE %IF %UPCASE(&LOT_REP) EQ NA %THEN
	%DO;
		&LOT_PROG = 0;
	%END;
...
**																					**;
**	If your date variable is not in SAS date format, the language below will 		**;
**	attempt to convert it.  If the conversion is not successful, please contact 	**;
**	a SAS support person for assistance.											**;
**************************************************************************************;
...
%MACRO G5_DATE_CONVERT;
		...
		DATA _NULL_;
			...
			DO;
				...
				SALEDATEFORM = VFORMATN(&DATE);
					IF SUBSTR(SALEDATEFORM,1,1) = '$' THEN SALEDATE_FORMAT_TYPE = 'CHARACTER';
					ELSE IF SALEDATEFORM IN( 'DATE','WEEK','YYMM', 'MMDD','WORD') THEN SALEDATE_FORMAT_TYPE = 'DATE';
					ELSE SALEDATE_FORMAT_TYPE = 'NUMERIC';
				%GLOBAL SALEDATE_FORMAT;
				CALL SYMPUT ('SALEDATE_FORMAT',SALEDATE_FORMAT_TYPE);
			END;
		RUN;

	%GLOBAL DATE_FORMAT&TYPE;
	...
	**------------------------------------------------------------------------------**;
	** 	Attempt to convert the sale date variable to date format, when necessary. 	**;
	**------------------------------------------------------------------------------**;
	...
	**------------------------------------------------------------------------------**;
	** 	Retest the format of converted date variable to see if conversion was 		**;
	**	successful.																	**;
	**------------------------------------------------------------------------------**;
		...
		DATA _NULL_;
			...
			SET &SALESDB;
				IF _N_=1;
					...
					IF CONVERTDATEFORM IN('DATE','WEEK','YYMM', 'MMDD','WORD') THEN CONVERTDATE = 'YES';
					ELSE CONVERTDATE = 'NO';
		...
		%DO;
			...
			PROC PRINT DATA = &SALESDB (OBS = &PRINTOBS) SPLIT="*";
				...
				TITLE6 "If orignal data is missing and the converted date is 01JAN1960, then variable &date is uninitialized.";
			RUN;
...
%MACRO G6_CHECK_SALES;
	...
	%GLOBAL MONTH;
	%LET MONTH = ;	/* Null value for macro variables &CMMONTH or &USMONTH for investigations. */
	...
	DATA &SALES NEGDATA_&DB OUTDATES_&DB;
		...
		ELSE DO;
    		...
    		**----------------------------------------------------------------------**;
			**	In administrative reviews, define CMMONTH and USMONTH variables so 	**;
	...
	%DO;
		%MACRO MARGIN_FILTER;
		%MEND MARGIN_FILTER;
	 	%CHECK_SALES(CMMONTH,&CMQTY,&CMGUP,&CMDATE,CMSALES,COMPARISON MARKET,CM);
	%END;
	%IF %UPCASE(&SALESDB) = DOWNSTREAM %THEN
	%DO;
		%MACRO MARGIN_FILTER;
		%MEND MARGIN_FILTER;
	 	%CHECK_SALES(CMMONTH,&CMQTY,&CMGUP,&CMDATE,DOWNSTREAM,DOWNSTREAM,DS);
	%END;
...
%MACRO G7_EXRATES;
	...
	**------------------------------------------------------------------------------**;
		...
		**--------------------------------------------------------------------------**;
			...
			**----------------------------------------------------------------------**;
				...
				DATA &SALESDB NORATES;
					...
					BY &DATE;
					IF A & B THEN OUTPUT &SALESDB;
					ELSE IF A & NOT B THEN OUTPUT NORATES;
				RUN;
	...
	%IF %UPCASE(&SALESDB) = CMSALES %THEN
	%DO;
		%MERGE_RATES(&USE_EXRATES1,&EXDATA1,EXRATE1,XRATE1,&CMDATE);
		%MERGE_RATES(&USE_EXRATES2,&EXDATA2,EXRATE2,XRATE2,&CMDATE);
	%END;
...
%MACRO G8_FIND_NOPRODUCTION;
		...
		%END;
		%ELSE %IF %UPCASE(&COST_PROD_CHARS) = NO %THEN
		%DO;
			%LET PROD_CHARS = ;
		%END;
		...
		%DO;
			...
			%DO;
				%MACRO NOPROD_TIME_TYPE;
					NOPROD_TIME_TYPE = "TS";
					IF &COST_TIME_PERIOD IN(&TIME_ANNUALIZED) THEN NOPROD_TIME_TYPE = "AN";
				%MEND NOPROD_TIME_TYPE;
			%END;
		...
		DATA _NULL_;
			SET NOPROD;
			IF _N_ = 1;
				IF NOBS GT 0 THEN MATCH_NOPRODS = "YES";
				ELSE IF NOBS = 0 THEN MATCH_NOPRODS = "NO";
	...
	%DO;
		...
		%MACRO GETCHARS;
			...
			%DO;
						...
						%MACRO RENAMECHARS;
							...
							%DO %UNTIL (%SCAN(&USCHAR, &I, %STR( )) = %STR());
								%LET RENAMECALC = &RENAMECALC
								 ...
								 = %SYSFUNC(COMPRESS(%SCAN(&CMCHAR,&I,%STR( )))) %NRSTR(;);
								 &RENAMECALC
								%LET I = %EVAL(&I + 1);
							%END;
						%MEND RENAMECHARS;
				...
				PROC SORT DATA = CMSALES  NODUPKEY OUT = CMCONNUMLIST (KEEP=&CMCPPROD &CMCHAR);
					BY &CMCPPROD;
				RUN;

				DATA CONNUMLIST;
					SET CMCONNUMLIST USCONNUMLIST;
				RUN;

				PROC SORT DATA = CONNUMLIST NODUPKEY;
					BY &CMCPPROD;
				RUN;
...
%MACRO G10_TIME_PROD_LIST(SALES_CHAR);
	...
	**------------------------------------------------------------------------------**;
	...
	PROC SORT DATA = TIMELIST NODUPKEY;
		BY &COST_TIME_PERIOD;
	RUN;
...
%MACRO G13_CALC_INDICES;
	...
	%MACRO INDICES(INPUT,INPUT_ADJUSTED,INDEX_GROUP);
		...
		PROC SORT DATA = INDEX_GRP NODUPKEY OUT=INDEXLIST_&INPUT (KEEP= &COST_MANF &COST_TIME_PERIOD &INDEX_GROUP
...
**************************************************************************************;
** G-15: CREATE &VCOM AND &TCOM FROM COST DATABASE OR FROM SALES DATA				**;
**************************************************************************************;
...
%MACRO G15_CHOOSE_COSTS;
	...
	%GLOBAL VCOM TCOM TOTALCOST;

	%MACRO COSTDATA(SALES_VCOM,SALES_TCOM,PRODCHARS);
		...
		%DO;
			...
			%DO;
				...
				%DO;
					...
					%END;
				%END;
				%ELSE %IF %UPCASE(&COMPARE_BY_TIME) = NO
				%THEN %DO;
					%MACRO TIME_TYPE_FORMAT;
					%MEND TIME_TYPE_FORMAT;
				%END;
				...
				**------------------------------------------------------------------**;
 				...
 				PROC SORT DATA = COST
					OUT = COST (KEEP= &COST_MANF &COST_MATCH &COST_TIME_TYPE &COST_TIME_PERIOD
        		     		&PRODCHAR &COST_QTY VCOMCOP TCOMCOP GNACOP INTEXCOP TOTALCOP);
 					BY &COST_MANF &COST_MATCH &COST_TIME_PERIOD;
				RUN;
	...
	**------------------------------------------------------------------------------**;
	** G-15-B: 	When there is no cost database, use variables in the sales dataset	**;
	**       	for &VCOM and &TCOM, when available.  				   				**;
	**------------------------------------------------------------------------------**;

	 	%IF %UPCASE(&USE_COSTDATA)=NO %THEN
	 	%DO;

			%LET TOTALCOST =  ;

			%IF %UPCASE(&SALES_VCOM) NE NA %THEN
			%DO;
				%LET VCOM = &SALES_VCOM;
			%END;
			%IF %UPCASE(&SALES_VCOM) = NA %THEN
			%DO;
				%LET VCOM = ;
			%END;
			%IF %UPCASE(&SALES_TCOM) NE NA %THEN
			%DO;
				%LET TCOM = &SALES_TCOM;
			%END;
			%IF %UPCASE(&SALES_TCOM) = NA %THEN
			%DO;
				%LET TCOM = ;
			%END;
	...
	%DO;
		%LET CALC_COST = YES;
		%COSTDATA(&CMVCOM,&CMTCOM,&CMCHAR);
	%END;
	%IF %UPCASE(&SALESDB) = USSALES %THEN
	%DO;
		%IF %UPCASE(&USE_COSTDATA) = NO %THEN
		%DO;
			%COSTDATA(&US_VCOM,&US_TCOM,&USCHAR);
		%END;
		%IF %UPCASE(&USE_COSTDATA) = YES %THEN
		%DO;
...
**************************************************************************************;
** G-17: MERGE COST DATA WITH SALES DATA. SAVE DATA FROM CM PROGRAM	    			**;
...
%MACRO G17_FINALIZE_COSTDATA;
	...
	%DO;
		...
		%MACRO MERGE_COST(SALES,SALES_MATCH,SALES_TIME,COSTDB);
			...
			DATA &SALES NOCOST;
				MERGE &SALES (IN=A) AVGCOST (IN=B RENAME = (&COST_MATCH = &SALES_MATCH
															&COST_MANF &EQUAL_COST_MANF &SALES_COST_MANF
															&COST_TIME_PERIOD &EQUAL_TIME &SALES_TIME));
				BY &SALES_MATCH &SALES_COST_MANF &SALES_TIME;
				IF A AND B THEN OUTPUT &SALES;
				ELSE IF A & NOT B THEN OUTPUT NOCOST;
			RUN;
		...
		%IF %UPCASE(&SALESDB) = CMSALES %THEN
		%DO;
			%MERGE_COST(CMSALES,&CMCPPROD,&CM_TIME_PERIOD,AVGCOST);
		...
		%IF %UPCASE(&SALESDB) = DOWNSTREAM %THEN
		%DO;
			%MERGE_COST(DOWNSTREAM,&CMCPPROD,&CM_TIME_PERIOD,AVGCOST);
		%END;
		%IF %UPCASE(&SALESDB) = USSALES %THEN
		%DO;
			%IF %UPCASE(&COST_TYPE) = CM %THEN
			%DO;
				%MERGE_COST(USSALES,&USCVPROD,&US_TIME_PERIOD,COMPANY.&RESPONDENT._&SEGMENT._&STAGE._COST);
			%END;
			%IF %UPCASE(&COST_TYPE) = CV %THEN
			%DO;
...
%MACRO G19_PROGRAM_RUNTIME;
	...
	%DO;
		...
		DATA _NULL_;
			...
			IF B_DATE = E_DATE
			THEN TOT_TIME = E_TIME - B_TIME;
			ELSE TOT_TIME = (86400 - B_TIME) + E_TIME;
...
**************************************************************************************;
** CM-1:  CREATE MACROS AND MACRO VARIABLES REGARDING PRIME/NON-PRIME MERCHANDISE	**;
**        AND MANUFACTURER DESIGNATION		   										**;
**************************************************************************************;

%MACRO CM1_PRIME_MANUF_MACROS;

	%GLOBAL CMMANF SALES_COST_MANF AND_SALES_MANF EQUAL_SALES_MANF
			COST_MANF AND_COST_MANF EQUAL_COST_MANF MANF_LABEL COP_MANF_OUT FIRST_COST_MANF
			CMPRIM PRIME_PRINT PRIME_LABEL PRIME_TITLE AND_PRIME EQUAL_PRIME ;
	...
	**------------------------------------------------------------------------------**;
	...
	%IF %UPCASE(&CMMANUF) = NA %THEN
	%DO;
		%LET CMMANF = ;				/* manufacturer for sales data */
	...
	**------------------------------------------------------------------------------**;
	...
	%IF %UPCASE(&CMMANUF) NE NA %THEN
	%DO;
		%LET CMMANF = &CMMANUF;
		%LET AND_SALES_MANF = AND ;
		%LET EQUAL_SALES_MANF = = ;
		%LET MANF_LABEL = &CMMANF = "MANUFACTURER*CODE *============" ;
	...
	**------------------------------------------------------------------------------**;
	...
	%IF %UPCASE(&CMPRIME) = NA %THEN
	%DO;
		%LET CMPRIM = ;			/* prime code for sales data */
	...
	**------------------------------------------------------------------------------**;
	...
	%DO;
		...
		%MACRO PRIME_PRINT;
			...
			PAGEBY &CMPRIM;
		%MEND PRIME_PRINT;
		%LET PRIME_LABEL = &CMPRIM = "PRIME/SECOND*QUALITY MDSE*============" ;
 	%END;

%MEND CM1_PRIME_MANUF_MACROS;

**************************************************************************************;
**  CM2: SPLIT MIXED-CURRENCY VARIABLES INTO SINGLE-CURRENCY VARIABLES   			**;
**************************************************************************************;

%MACRO CM2_MIXEDCURR(SALES);
...
**************************************************************************************;
** CM-3: ARMS-LENGTH TEST OF AFFILIATED PARTY SALES									**;
**************************************************************************************;

%MACRO CM3_ARMSLENGTH;

	%IF %UPCASE(&RUN_ARMSLENGTH) = YES %THEN
	%DO;

		**--------------------------------------------------------------------------**;
		** 	Create the macro variables, as needed, for non-affiliated manufacturer	**;
		** 	and prime.                      										**;
		**--------------------------------------------------------------------------**;

	%GLOBAL NAFMANF NAFMANF_DEF NAFPRIME NAFPRIME_DEF AFFVCOM AFFTCOM NAFVCOM CHK4SIM NAFVCOM_DEF
			MAKE_NAFCHARS NAFCHAR DIFCHAR;

		%IF %UPCASE(&CMMANUF) = NA %THEN		/* Create null values when manufacturer is not relevant.  */
		%DO;
			%LET NAFMANF =;
			%MACRO NAFMANF_DEF;
			%MEND NAFMANF_DEF;
		...
		%ELSE %IF %UPCASE(&CMMANUF) NE NA %THEN	/* Create values when manufacturer is relevant  */
		%DO;
			%LET NAFMANF = NAFMANF;
			%MACRO NAFMANF_DEF;
				NAFMANF = &CMMANF;
		...
		%IF %UPCASE(&CMPRIME) = NA %THEN  	/* Create null values when prime/non-prime is not relevant. */
		%DO;
				%LET NAFPRIME = ;
				%MACRO NAFPRIME_DEF;
				%MEND NAFPRIME_DEF;
		...
		%ELSE %IF %UPCASE(&CMPRIME) NE NA %THEN 	/* Create null values when prime/non-prime is relevant, */
		%DO;
		...
		**--------------------------------------------------------------------------**;
		...
		/* 	set macro values when have cost data  */

		%IF %UPCASE(&USE_COSTDATA) = YES OR (%UPCASE(&CMVCOM) NE NA AND %UPCASE(&CMTCOM) NE NA) %THEN
		%DO;
			%LET AFFVCOM = &VCOM;
			%LET AFFTCOM = &TCOM;
			%LET NAFVCOM = NAFVCOM;
			...
			%MACRO MAKE_NAFCHARS;
				...
				%LET NAFCHAR = ;
				%LET DIFCHAR = ;
				%DO %UNTIL (%SCAN(&CMCHAR, &I, %STR( )) = %STR());
					%LET RENAMENAF = &RENAMENAF
					RENAME %SYSFUNC(COMPRESS(%SCAN(&CMCHAR,&I,%STR( ))))
					= %SYSFUNC(COMPRESS(%SCAN(&CMCHAR,&I,%STR( ))))_NAF %NRSTR(;);
					&RENAMENAF
					%LET NAFCHAR = &NAFCHAR
					%SYSFUNC(COMPRESS(%SCAN(&CMCHAR,&I,%STR( ))))_NAF;
					%LET DIFCHAR = &DIFCHAR
					%SYSFUNC(COMPRESS(%SCAN(&CMCHAR,&I,%STR( ))))_DIF;
					%LET I = %EVAL(&I + 1);
				%END;
			%MEND MAKE_NAFCHARS;
		%END;

		/* set cost macro values when no cost data available */

		%ELSE %IF %UPCASE(&USE_COSTDATA) = NO OR (%UPCASE(&CMVCOM) = NA OR %UPCASE(&CMTCOM) = NA) %THEN
		%DO;
			%LET AFFCOST =;
			%LET AFFVCOM =;
			%LET NAFVCOM =;
		...
		DATA CMAFF (KEEP=&CMCUST &CMCONNUM &CMMANF &CMPRIM CMLOT
               		&CM_TIME_PERIOD &CMQTY CMNETPRI &AFFVCOM &AFFTCOM &CMCHAR)
             CMNAF (KEEP=&CMCONNUM CMLOT &CMMANF &CMPRIM
              		&CM_TIME_PERIOD &CMQTY CMNETPRI &NAF_TIME_PERIOD NAFCONN NAFLOT
                    &AFFVCOM &NAFMANF &NAFPRIME &CMCHAR);
 			SET CMSALES;
			IF &CMAFFL = &NAFVALUE THEN
			DO;
				NAFCONN = &CMCONNUM;
				NAFLOT = CMLOT;
				%NAFMANF_DEF
				%NAFPRIME_DEF
				&NAF_TIME_PERIOD &EQUAL_TIME &CM_TIME_PERIOD ;
				OUTPUT CMNAF;
			END;
			ELSE OUTPUT CMAFF;
		RUN;
		...
		**--------------------------------------------------------------------------**;
		** CM3-A: Check Customer Affiliation and LOT								**;
		**--------------------------------------------------------------------------**;

		PROC SORT DATA = CMSALES;
			BY &CMAFFL CMLOT &CMMANF &CMPRIM &CMCUST;
		RUN;

		PROC MEANS NOPRINT DATA = CMSALES;
			BY &CMAFFL CMLOT &CMMANF &CMPRIM ;
			VAR &CMQTY;
			OUTPUT OUT = TOTS (DROP=_TYPE_ _FREQ_)
			       N=SALES SUM=TOTQTY;
		RUN;

		PROC SORT DATA = CMSALES OUT = NUMCUST NODUPKEY;
 			BY &CMAFFL CMLOT &CMMANF &CMPRIM &CMCUST;
		RUN;
		...
		PROC PRINT DATA = TOTCUST SPLIT='*';
			VAR &CMAFFL CMLOT &CMMANF &CMPRIM NCUST SALES TOTQTY;
			SUM NCUST SALES TOTQTY;
			FORMAT SALES COMMA7. TOTQTY COMMA12.;
 			LABEL &CMAFFL  = "CUSTOMER *AFFILIATION*==========="
			      CMLOT   = "LEVEL(S)*OF TRADE*========"
			      &MANF_LABEL
			      &PRIME_LABEL
		...
		**--------------------------------------------------------------------------**;
		** CM3-B: Weighted-average net prices of affiliated customers				**;
		**--------------------------------------------------------------------------**;

		PROC SORT DATA = CMAFF;
			BY &CMCUST CMLOT &CMMANF &CMPRIM &CM_TIME_PERIOD &CMCONNUM;
		RUN;

		PROC MEANS NOPRINT DATA = CMAFF;
			BY &CMCUST CMLOT &CMMANF &CMPRIM &CM_TIME_PERIOD &CMCONNUM;
			ID &AFFVCOM &AFFTCOM &CMCHAR;
			VAR CMNETPRI;
			WEIGHT &CMQTY;
		...
		PROC SORT DATA=TOTAFF;
			BY CMLOT &CMMANF &CMPRIM &CM_TIME_PERIOD &CMCONNUM &CMCUST;
		RUN;

		**--------------------------------------------------------------------------**;
		** CM3-C: Weight-average net prices of non-affiliated customers				**;
		**--------------------------------------------------------------------------**;

		PROC SORT DATA = CMNAF;
			BY CMLOT &CMMANF &CMPRIM &CM_TIME_PERIOD &CMCONNUM;
		RUN;

		PROC MEANS NOPRINT DATA = CMNAF;
			BY CMLOT &CMMANF &CMPRIM &CM_TIME_PERIOD &CMCONNUM;
		...
		**--------------------------------------------------------------------------**;
		** CM3-D: Identify identical product matches at the same LOT				**;
		**--------------------------------------------------------------------------**;
		...
		DATA ALLCOMP NOCOMP;
			...
			DO;
				...
				OUTPUT ALLCOMP;
			END;
			ELSE IF A & NOT B THEN OUTPUT NOCOMP;
		RUN;

		**--------------------------------------------------------------------------**;
		** CM3-E: Identify similar product matches at the same LOT only if cost 	**;
		**        data are available 							                    **;
		**--------------------------------------------------------------------------**;
		...
		%MACRO CM3_SIM;
			...
			%DO;
				DATA SIMMATCH;
					...
					DO J=1 TO LAST;
						SET TOTNAF (DROP=CMLOT &CMMANF &CMPRIM
							        &CM_TIME_PERIOD &CMCONNUM)
						POINT=J NOBS=LAST;
						LOTDIFF = ABS(NAFLOT - CMLOT);
						IF &AFFVCOM GT .  AND &NAFVCOM GT .
						THEN DIFMER = &NAFVCOM - &AFFVCOM;
						ELSE DIFMER = .;

						IF DIFMER GT .
						THEN COSTDIFF = ABS(DIFMER/&AFFTCOM);
						ELSE COSTDIFF = .;

						**----------------------------------------------------------**;
						** CM3-F Examine all product matches within the 20pct		**;
						**       DIFMER test and at the same LOT					**;
						**----------------------------------------------------------**;

						IF 0.20 GE COSTDIFF GT .
						   	& LOTDIFF  = 0
						   	&AND_SALES_MANF &NAFMANF &EQUAL_SALES_MANF &CMMANF
							&AND_PRIME &NAFPRIME &EQUAL_PRIME &CMPRIM
						   	&AND_TIME &CM_TIME_PERIOD &EQUAL_TIME &NAF_TIME_PERIOD
						THEN
						DO;

							**------------------------------------------------------**;
							** CM3-G: Select similar products based on product		**;
							**        characteristics.								**;
							**------------------------------------------------------**;
				...
				PROC SORT DATA=SIMMATCH;
					BY CMLOT &CMMANF &CMPRIM &CMCUST &CM_TIME_PERIOD &CMCONNUM
				...
				DATA SIMCOMP (DROP=&DIFCHAR) TOP5SIM;
					...
					IF FIRST.&CMCONNUM THEN CHOICE = 0;
					CHOICE + 1;
					IF CHOICE = 1 THEN OUTPUT SIMCOMP;
					IF CHOICE LE 5 THEN OUTPUT TOP5SIM;
				RUN;
				...
				PROC PRINT DATA = TOP5SIM (OBS = 15);
					BY CMLOT &CMMANF &CMPRIM &CMCUST &CM_TIME_PERIOD &CMCONNUM;
					PAGEBY &CMCONNUM;
					VAR &CMCHAR NAFLOT &NAF_TIME_PERIOD NAFCONN &NAFCHAR
					    &AFFVCOM &AFFTCOM &NAFVCOM DIFMER
					    MATCH DIFCODE LOTDIFF COSTDIFF CHOICE;
					TITLE3 'CHECK TOP 5 SIMILAR MATCHES (NAFADJPR INCLUDES DIFMER)';
				RUN;

				**------------------------------------------------------------------**;
				** CM3-H: Combine identical and similar matches						**;
				**------------------------------------------------------------------**;
		...
		**--------------------------------------------------------------------------**;
		** CM3-I: For each product sold to an affiliate with an identical or		**;
		...
		**--------------------------------------------------------------------------**;
		...
		PROC PRINT DATA=ALLCOMP (OBS=&PRINTOBS);
			BY &CMCUST;
			ID &CMCUST;
				VAR CMLOT &CMMANF &CMPRIM &CM_TIME_PERIOD &CMCONNUM
					AFFOBS AFFQTY AFFNETPR &AFFVCOM &AFFTCOM
		...
		**--------------------------------------------------------------------------**;
		** CM3-J: Calculate the overall CUSRATIO for each affiliated customer, and	**;
		...
		**--------------------------------------------------------------------------**;
		...
		PROC SORT DATA=AFFCUST;
			BY &CMCUST;
		RUN;

		PROC PRINT DATA=AFFCUST (OBS=45) SPLIT='*';
			VAR &CMCUST AFFOBS NUMCOMP CUSRATIO RESULTS;
			SUM NUMCOMP AFFOBS PASSOBS FAILOBS;
			LABEL &CMCUST  = 'AFFILIATED*CUSTOMER(S)*==========='
		...
		**--------------------------------------------------------------------------**;
		** CM3-K: Discard HM sales that fail the CUSRATIO test						**;
		**--------------------------------------------------------------------------**;

		PROC SORT DATA = AFFCUST OUT = FAIL (KEEP=&CMCUST CUSRATIO);
			WHERE RESULTS = 'FAIL';
			BY &CMCUST;
		RUN;
		...
		DATA CMSALES HMAFFOUT;
			MERGE CMSALES (IN=INA) FAIL (IN=INB);
			BY &CMCUST;
			IF INA AND INB
...
**************************************************************************************;
** CM-4: CM VALUES FOR CEP PROFIT CALCULATIONS										**;
**************************************************************************************;
...
%MACRO CM4_CEPTOT;
	...
	%IF %UPCASE(&USE_COSTDATA) = YES %THEN
	%DO;
		%IF %UPCASE(&RUN_CMCEPTOT) = YES %THEN
		%DO;

			DATA CMSALES;
				SET CMSALES;
					REVENUH  = (CMGUP + CMGUPADJ - CMDISREB)*&CMQTY;
					COGSH    = (AVGCOST + CMPACK)*&CMQTY;
					SELLEXPH = (CMDSELL + CMISELL + CMCOMM)*&CMQTY;
					MOVEEXPH = CMMOVE*&CMQTY;
			RUN;

			PROC MEANS NOPRINT DATA=CMSALES;
				VAR REVENUH COGSH SELLEXPH MOVEEXPH;
				OUTPUT OUT=COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CMCEP (DROP=_FREQ_ _TYPE_)
				       SUM=TOTREVH TOTCOGSH TOTSELLH TOTMOVEH;
			RUN;

			PROC PRINT DATA=COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CMCEP;
...
%MACRO CM5_COSTTEST;
	...
	%IF %UPCASE(&USE_COSTDATA) = NO %THEN
	%DO;
		%LET RUN_COSTTEST = NO;  /* If required, set the cost test not to run when there is no cost database. */
	%END;

	%IF %UPCASE(&RUN_COSTTEST) = YES %THEN
	%DO;

		DATA CMSALES;
			SET CMSALES;
			IF CMNPRICOP GE AVGCOST THEN
			DO;
				QABOVE  = &CMQTY;
				QBELOW  = 0;
				OABOVE  = 1;
		...
		**--------------------------------------------------------------------------**;
		** CM5-A: Cost test results for each product								**;
		**--------------------------------------------------------------------------**;

		PROC SORT DATA=CMSALES;
			BY &CMMANF &CMPRIM &CMCONNUM;
		RUN;

		DATA CMCOP (KEEP=&CMMANF &CMPRIM &CMCONNUM OBSABOVE OBSBELOW
		                     QTYABOVE QTYBELOW PCTQABOV);
			SET CMSALES;
			BY &CMMANF &CMPRIM &CMCONNUM;

			IF FIRST.&CMCONNUM THEN
			DO;
				QTYABOVE = 0;
				OBSABOVE = 0;
				QTYBELOW = 0;
			...
			IF LAST.&CMCONNUM THEN
			DO;
				IF QTYABOVE=0
				THEN PCTQABOV=0;
				ELSE PCTQABOV=(QTYABOVE/(QTYABOVE+QTYBELOW))*100;
				OUTPUT CMCOP;
			END;
		RUN;

		PROC SORT DATA=CMCOP OUT=SUMMCTEST;
			BY &CMPRIM &CMCONNUM;
			RUN;
		...
		%MACRO PRIME_SUMMARY;
			...
			%DO;
				...
				DATA SUMMCTEST;
					SET SUMMCTEST;
					BY &CMPRIM;
					IF FIRST.&CMPRIM THEN COUNT=0;
					COUNT+1;
					IF COUNT LE 20 THEN OUTPUT SUMMCTEST;
				RUN;
		...
		PROC PRINT DATA = SUMMCTEST (OBS=&PRINTOBS);
			%PRIME_PRINT
			VAR &CMCONNUM QTYABOVE OBSABOVE QTYBELOW OBSBELOW PCTQABOV;
			TITLE3 "SAMPLE RESULTS OF COST TEST BY &CMCONNUM";
		RUN;

	**------------------------------------------------------------------------------**;
	** CM5-A-2: Cost test results for time periods									**;
	**------------------------------------------------------------------------------**;
	...
	%DO;
		...
		PROC SORT DATA=CMSALES;
			BY &CMMANF &CMPRIM &CM_TIME_PERIOD;
		RUN;

		DATA CMCOP_TIME (KEEP=&CMMANF &CMPRIM &CM_TIME_PERIOD OBSABOVE OBSBELOW
		                     QTYABOVE QTYBELOW PCTQABOV);
			SET CMSALES;
			BY &CMMANF &CMPRIM &CM_TIME_PERIOD;

			IF FIRST.&CM_TIME_PERIOD THEN
			DO;
				QTYABOVE = 0;
				OBSABOVE = 0;
				QTYBELOW = 0;
		...
		PROC SORT DATA=CMCOP_TIME OUT=SUMMCTEST_TIME;
			BY &CMPRIM &CM_TIME_PERIOD;
			RUN;
		...
		%DO;
			...
			DATA SUMMCTEST_TIME;
				...
				IF FIRST.&CMPRIM THEN COUNT=0;
				COUNT+1;
				IF COUNT LE 20 THEN OUTPUT SUMMCTEST_TIME;
			RUN;
		...
		PROC PRINT DATA = SUMMCTEST_TIME (OBS=&PRINTOBS);
			%BYLINE
			%IDLINE
			VAR &CM_TIME_PERIOD QTYABOVE OBSABOVE QTYBELOW OBSBELOW PCTQABOV;
			SUM QTYABOVE OBSABOVE QTYBELOW OBSBELOW;
			TITLE3 "SAMPLE RESULTS OF COST TEST BY &CM_TIME_PERIOD";
		RUN;
	...
	**--------------------------------------------------------------------------**;
	** CM5-B: Apply cost test results to create above- and below-cost databases	**;
	**--------------------------------------------------------------------------**;

		DATA CMABOVE CMBELOW
			 CMSALES (KEEP=&CMMANF &CMCONNUM &CMPRIM COSTTYPE &CMQTY CMNETPRI CMNPRICOP AVGCOST);
			MERGE CMSALES (IN=INA) CMCOP;
			BY &CMMANF &CMPRIM &CMCONNUM;
			IF INA;
		...
		PROC PRINT DATA=CMBELOW(OBS=&PRINTOBS);
			TITLE3 'SAMPLE OF BELOW COST HM SALES';
		RUN;

		PROC PRINT DATA=CMABOVE(OBS=&PRINTOBS);
	...
	**------------------------------------------------------------------------------**;
	** CM5-C: Cost Recovery Test for Time-Specific Costs							**;
	**																				**;
	**	For below-cost sales, compare the CONNUM average price across all time		**;
	**	periods to the CONNUM average cost across all time periods. If the average	**;
	**	price exceeds the average cost, then below-cost sales of that CONNUM will 	**;
	**	reclassified as above cost.													**;
	**------------------------------------------------------------------------------**;
	...
	%DO;
		...
		PROC MEANS NOPRINT DATA = CMSALES;
			...
			VAR CMNPRICOP;
			WEIGHT &CMQTY;
			OUTPUT OUT = CONNUMPRICE (DROP=_FREQ_ _TYPE_) MEAN = CONNUM_PRICE;
		RUN;

		DATA CMBELOW4TEST;
			MERGE CMBELOW(IN=A) CONNUMPRICE (IN=B);
			BY &CMMANF &CMPRIM &CMCONNUM;
			IF A & B;
			IF &CM_TIME_PERIOD IN(&LIST_TIMES);
		RUN;

		PROC SORT DATA = CMBELOW4TEST;
			BY &CMMANF &CMCPPROD;
		RUN;

		DATA CMBELOW4TEST RECOVERED;
			MERGE CMBELOW4TEST (IN=A)
				CONNUMCOST (IN=B RENAME=(&COP_MANF_OUT &EQUAL_COST_MANF &CMMANF COST_MATCH=&CMCPPROD));
			BY &CMMANF &CMCPPROD;
			IF A & B;
				FORMAT RECOVERED $3.;
		...
		DATA RECOVERTYPE;
			SET CMBELOW4TEST;
			BY RECOVERED &CMMANF &CMPRIM &CMCONNUM &CM_TIME_PERIOD;
			IF FIRST.RECOVERED THEN COUNT = 0;
			COUNT + 1;
			IF COUNT LE 20 THEN OUTPUT RECOVERTYPE;
		RUN;

		PROC SORT DATA = RECOVERTYPE;
			BY  &CMMANF &CMPRIM &CMCONNUM;
		RUN;

		PROC PRINT DATA = RECOVERTYPE SPLIT="*";
			BY  &CMMANF &CMPRIM &CMCONNUM;
			ID  &CMMANF &CMPRIM &CMCONNUM;
			VAR &CM_TIME_PERIOD CMNPRICOP AVGCOST CONNUM_PRICE CONNUM_COST RECOVERED COSTTYPE;
			LABEL	RECOVERED  = "WAS SALE*RECOVERED?"
					&CMCONNUM = "CONTROL*NUMBER"
					&CM_TIME_PERIOD = "TIME*PERIOD"
					CMNPRICOP = "TRANSACTION*PRICE"
		...
		DATA CMABOVE;
			SET CMABOVE RECOVERED (DROP=RECOVERED CONNUM_PRICE CONNUM_COST);
		RUN;
...
**************************************************************************************;
** CM-6: SELECT CM DATA FOR WEIGHT AVERAGING										**;
**																					**;
**  Use all CM sales when there is no cost test, or above-cost CM sales when        **;
**  there is a cost test.                                        					**;
**************************************************************************************;

%MACRO CM6_DATA_4_WTAVG;
	%IF %UPCASE(&RUN_COSTTEST) = YES %THEN
	%DO;
		PROC SORT DATA=CMABOVE OUT = CM;
		BY &CMMANF &CMPRIM CMLOT &MONTH &CMCONNUM &CM_TIME_PERIOD;
		RUN;
	%END;
	%ELSE
	%DO;
		PROC SORT DATA=CMSALES OUT = CM;
			BY &CMMANF &CMPRIM CMLOT &MONTH &CMCONNUM &CM_TIME_PERIOD;
		RUN;
	%END;
%MEND CM6_DATA_4_WTAVG;

**************************************************************************************;
** CM-7: WEIGHT AVERAGE CM DATA														**;
**																					**;
**	Rename control number, VCOM, manufacturer, prime/non-prime, where applicable,  	**;
**	using standardized names.														**;
**************************************************************************************;

%MACRO CM7_WTAVG_DATA;

	%GLOBAL MANUF_RENAME PRIME_RENAME VCOST_RENAME TIME_PER_RENAME TIME_PERIOD_RENAME;
	...
	%DO;
		...
		%DO;
			%LET TIME_PER_RENAME = &CM_TIME_PERIOD=CM_TIME_PERIOD;
			%MACRO TIME_PERIOD_RENAME;
				RENAME &CM_TIME_PERIOD=CM_TIME_PERIOD;
			%MEND TIME_PERIOD_RENAME;
		%END;
	%END;
	...
	%DO;
		...
		%IF %UPCASE(&CMVCOM) EQ NA %THEN
		%DO;
			%MACRO VCOST_RENAME;
			%MEND VCOST_RENAME;
		%END;
		%IF %UPCASE(&CMVCOM) NE NA %THEN
		%DO;
			%MACRO VCOST_RENAME;
				RENAME &VCOM=CMVCOM;
			%MEND VCOST_RENAME;
	...
	%DO;
		%MACRO MANUF_RENAME;
			RENAME &CMMANF=CMMANF;
		%MEND MANUF_RENAME;
	%END;

	%IF %UPCASE(&CMMANUF) EQ NA %THEN
	%DO;
		%MACRO MANUF_RENAME;
		%MEND MANUF_RENAME;
	%END;

	%IF %UPCASE(&CMPRIME) EQ NA %THEN
	%DO;
		%MACRO PRIME_RENAME;
		%MEND PRIME_RENAME;
	%END;
...
PROC PRINT DATA = COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CMWTAV (OBS = &PRINTOBS);
	TITLE3 "SAMPLE OF WEIGHT-AVERAGED CM VALUES FOR PRICE-TO-PRICE COMPARISON WITH U.S. SALES";
RUN;

%MEND CM7_WTAVG_DATA;

**************************************************************************************;
** CM-8: CALCULATE SELLING EXPENSE AND PROFIT RATIOS FOR CV COMPARISONS				**;
**************************************************************************************;
...
%MACRO CM8_CVSELL;
	...
	%DO;
		...
		PROC MEANS NOPRINT DATA = CM;
			BY CMLOT;
			VAR CMDSELL CMISELL CMCOMM CMCRED CMICC
			    CMINDCOM CMNPRICOP CVCREDPR AVGCOST;
			WEIGHT &CMQTY;
			OUTPUT OUT=CVSELLOT (DROP=_FREQ_ _TYPE_)
			       SUM=DSELCV ISELCV COMMCV CREDCV INVCV
			           ICOMCV CMTOTVAL CMTOTCVP CMTOTCOP;
		RUN;

		DATA COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CVSELL (KEEP=CMLOT PRATECV
															DSELCVR ISELCVR COMMCVR
		                             						CREDCVR INVCVR ICOMCVR);
			SET CVSELLOT;

			DSELCVR = DSELCV/CMTOTCOP;
			ISELCVR = ISELCV/CMTOTCOP;
			ICOMCVR = ICOMCV/CMTOTCOP;
			COMMCVR = COMMCV/CMTOTCOP;
			CREDCVR = CREDCV/CMTOTCVP;
			INVCVR  = INVCV/CMTOTCOP;

			PRATECV = (CMTOTVAL-CMTOTCOP)/CMTOTCOP;
			IF PRATECV LT 0
			THEN PRATECV=0;
		RUN;
...
**************************************************************************************;
** CM-9: CALCULATE LEVEL OF TRADE ADJUSTMENT										**;
**************************************************************************************;

%MACRO CM9_LOTADJ;

	%IF %UPCASE(&RUN_CMLOTADJ) = YES %THEN
	%DO;
		PROC SORT DATA = CM (KEEP=CMLOT &CMCONNUM &CM_TIME_PERIOD CMNETPRI &CMQTY
							 RENAME=(&CMCONNUM=CMCONNUM ))
			      OUT=LOTS;
		BY CMLOT CMCONNUM &CM_TIME_PERIOD;
		RUN;
		...
		BY CMLOT CMCONNUM &CM_TIME_PERIOD;
			...
			OUTPUT OUT=LOT1 (DROP=_FREQ_ _TYPE_)
			       MEAN=CMPRICE SUMWGT=CMQTY;
		RUN;

		DATA LOT2;
			SET LOT1;
			RENAME CMLOT = USLOT CMCONNUM = USCONNUM &CM_TIME_PERIOD &EQUAL_TIME &US_TIME
			       CMPRICE = USPRICE CMQTY = USQTY;
		RUN;

		DATA DIFF (KEEP=CMLOT CMCONNUM USCONNUM USLOT &CM_TIME_PERIOD &US_TIME
			...
			IF USCONNUM=CMCONNUM &AND_TIME &CM_TIME_PERIOD &EQUAL_TIME &US_TIME THEN
				DO;
					DIFF  = (USPRICE-CMPRICE)/CMPRICE;
					QTY   = USQTY+CMQTY;
					GNUM  = 0;
					GQTY  = 0;
					LNUM  = 0;
					...
					IF USPRICE GT CMPRICE THEN
					DO;
						GNUM = 1;
						GQTY = USQTY+CMQTY;
					END;
					ELSE
					IF USPRICE LT CMPRICE THEN
					DO;
						LNUM = 1;
						LQTY = USQTY+CMQTY;
					END;
	...
	DATA COMPANY.&RESPONDENT._&SEGMENT._&STAGE._LOTADJ (KEEP=USLOT CMLOT LOTADJ);
			SET RESULTS;
		RUN;
	%END;
%MEND CM9_LOTADJ;
...
**************************************************************************************;
** US-1:  CREATE MACROS AND MACRO VARIABLES REGARDING PRIME/NON-PRIME MERCHANDISE,	**;
**        MANUFACTURER DESIGNATION, COST AND MONTHS									**;
**************************************************************************************;

%MACRO US1_MACROS;

	%GLOBAL USMANF CMMANF AND_P2P_MANF EQUAL_P2P_MANF MANF_LABEL
			SALES_COST_MANF COST_MANF AND_COST_MANF EQUAL_COST_MANF
			USPRIM CMPRIM EQUAL_PRIME AND_PRIME PRIME_LABEL PRIME_TITLE
			USMON CMMON DE_MINIMIS;
	...
	**------------------------------------------------------------------------------**;
	...
	%DO;
		...
		%LET CMMANF = ;				/* CM sales manufacturer for merging with U.S. sales */
	...
	**------------------------------------------------------------------------------**;
	...
	%DO;
		...
		**--------------------------------------------------------------------------**;
		**  1-B-ii-a Create macros when U.S. sales manufacturer is relevant 		**;
		**			but CM manufacturer is not.										**;
		**--------------------------------------------------------------------------**;

		%IF %UPCASE(&CMMANUF=NO) %THEN
		%DO;
			%LET USMANF = ;
			%LET CMMANF = ;
			%LET AND_P2P_MANF = ;
		...
		**			and CM manufacturer is also relevant.							**;
		**--------------------------------------------------------------------------**;

		%IF %UPCASE(&CMMANUF=YES) %THEN
		%DO;
			%LET USMANF = &USMANUF;
			%LET CMMANF = CMMANF;
		...
		**--------------------------------------------------------------------------**;
		**  1-B-ii-c Create macros when U.S. sales manufacturer is relevant and 	**;
		**			cost data is not being used.									**;
		**--------------------------------------------------------------------------**;
		...
		**--------------------------------------------------------------------------**;
		**  1-B-ii-d Create macros when U.S. sales manufacturer is relevant			**;
		**			and cost data is being used.									**;
		**--------------------------------------------------------------------------**;
		...
		%DO;
			...
			**----------------------------------------------------------------------**;
			**  Create macros when cost data comes from the CM program				**;
			**----------------------------------------------------------------------**;
	...
	**------------------------------------------------------------------------------**;
	**  1-C CREATE MACROS WHEN COST DATA COMES FROM THE CM PROGRAM					**;
	**------------------------------------------------------------------------------**;
	...
	**------------------------------------------------------------------------------**;
	...
	%DO;
		...
		%LET CMPRIM = ;			/* prime code for CM sales data */
		%LET AND_PRIME = ; 		/* AND operator for prime v nonprime purposes */
		%LET EQUAL_PRIME = ; 	/* EQUAL operator for prime v nonprime purposes */
		%LET PRIME_TITLE = ; 	/* prime v nonprime text for titles */
		%LET PRIME_LABEL = ;	/* printing label for prime code */
	...
	%DO;
		...
		%DO;
			...
			DATA USCEP;
				...
				**------------------------------------------------------------------**;
				** 5-A: Convert COGSU, REVENU, SELLEXPU, and MOVEU into CM currency.**;
			...
			DATA CEPTOT;
				SET USCEPTOT;
				IF _N_=1 THEN SET COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CMCEP;
...
%MACRO US6_ENTVALUE;
	...
	%DO;
		...
		DATA USSALES;
			...
			%ELSE %IF %UPCASE(&ENTERVAL) NE NA %THEN
			%DO;
...
**************************************************************************;
** US-7: FIND BEST CM MATCHES FOR U.S. SALES 							**;
**************************************************************************;
...
%DO;
	...
	%GLOBAL CMVCOM;	/* Default blank value for macro CMVCOM when no CM variable cost data is available */
	%IF %UPCASE(&CMVCOST) = YES %THEN
	%DO;
		%LET CMVCOM = CMVCOM;  /* Reset macro value when variable cost is available */
	%END;
	%IF %UPCASE(&CMVCOST) = NO %THEN
	%DO;
		%LET CMVCOM = ;
	%END;

	**------------------------------------------------------------------**;
	** 7-A: CREATE LISTS OF CM AND U.S. PRODUCTS FOR MATCHING			**;
	**------------------------------------------------------------------**;

	DATA CMSALES;
		SET COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CMWTAV;
	RUN;

	PROC SORT DATA = CMSALES OUT = CMMODELS;
		BY &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM;
	RUN;

	PROC PRINT DATA = CMMODELS (OBS = &PRINTOBS);
		VAR &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM &CMCHAR &CMVCOM;
		TITLE3 "SAMPLE OF COMPARISON MARKET PRODUCTS FOR CONCORDANCE";
	RUN;

	PROC SORT DATA = USSALES OUT = USMODELS (KEEP=&USMANF &USPRIM USLOT &US_TIME_PERIOD
			&USMON &USCONNUM &USCHAR &USVCOM &USTCOM &TOTALCOST) NODUPKEY;
		BY &USMANF &USPRIM USLOT &US_TIME_PERIOD &USMON &USCONNUM;
	RUN;

	PROC PRINT DATA = USMODELS (OBS = &PRINTOBS);
		VAR &USMANF &USPRIM USLOT &US_TIME_PERIOD &USMON &USCONNUM &USCHAR &USVCOM &USTCOM;
		TITLE3 'SAMPLE OF U.S. PRODUCTS FOR CONCORDANCE';
	RUN;

	**------------------------------------------------------------------**;
	** 7-B: MACRO FOR WINDOW PERIOD PREFERENCES IN ADMIN. REVIEWS		**;
	**------------------------------------------------------------------**;
	...
	**------------------------------------------------------------------**;
	...
	DATA IDMODELS;
		SET USMODELS;
		DO J=1 TO LAST;
		SET CMMODELS POINT=J NOBS=LAST;

			IF &USCONNUM = CMCONNUM THEN	/* Limits matches to identical */
			DO;

				NVMATCH = 3;	/* Default value, equivalent to CV if no identical or similar match found */
				LOTDIFF = ABS(USLOT - CMLOT);
				DIFMER = 0;
				COSTDIFF = 0;
				...
				**------------------------------------------------------**;
				** US7-C-i: IMPOSE CONSTRAINTS ON POSSIBLE IDENTICAL 	**;
				**			MATCHES										**;
	 			**------------------------------------------------------**;

				IF WNDORDER LE 6 										/* sales are contemporaneous  */
				&AND_P2P_MANF &USMANF &EQUAL_P2P_MANF &CMMANF			/* same manufacturer */
				&AND_PRIME &USPRIM &EQUAL_PRIME &CMPRIM 				/* same prime/non-prime indicator */
		   		&AND_TIME &US_TIME_PERIOD &EQUAL_TIME &CM_TIME_PERIOD	/* same cost-related time periods */
				THEN
				DO;
					NVMATCH = 1;
					OUTPUT IDMODELS;
	...
	**------------------------------------------------------------------**;
	...
	PROC CONTENTS NOPRINT DATA = OTHERMODELS
		OUT = OTHERMATCH (KEEP=NOBS);
	RUN;
	...
	DATA _NULL_;
		SET OTHERMATCH;
		IF _N_ = 1;
			IF NOBS GT 0 THEN FIND_SIMS = "YES";
			ELSE IF NOBS = 0 THEN FIND_SIMS = "NO";
	...
	**																	**;
	**	Information on VCOM and TCOM for U.S. sales, and VCOM for CM	**;
	...
	**------------------------------------------------------------------**;
	...
	%DO;
		...
		%IF %UPCASE(&USE_COSTDATA)=YES OR
			(%UPCASE(&USTCOM) NE NA AND %UPCASE(&USVCOM) NE NA)
			AND %UPCASE(&CMVCOST) = YES %THEN
		%DO;
			...
			**----------------------------------------------------------**;
			...
			DATA SIMMODELS;
				...
				SET CMMODELS POINT=J NOBS=LAST;
					...
					IF &CMVCOM GT .  AND &USVCOM GT .
					THEN DIFMER = &CMVCOM - &USVCOM;
					ELSE DIFMER = .;
					IF DIFMER GT .
					THEN COSTDIFF = ABS(DIFMER/&USTCOM);  /* difmer ratio */
					ELSE COSTDIFF = .;
					...
					**----------------------------------------------------------**;
					** 7-E-ii-b: CALCULATE DIFFERENCES IN PRODUCT				**;
					**			 CHARACTERISTIC VALUES							**;
					**----------------------------------------------------------**;

						DO I=1 TO DIM(DIFCHR);
							DIFCHR(I) = ABS(USCHR(I)-CMCHR(I));
						END;
						DROP I;
						NVMATCH = 2;
						OUTPUT SIMMODELS;
	...
	%END;
		...
		**------------------------------------------------------------------**;
		...
		DATA P2PMODS P2PTOP5;
			...
			IF FIRST.&USCONNUM THEN CHOICE=0;
			CHOICE + 1;
			IF CHOICE = 1 THEN OUTPUT P2PMODS;
			IF CHOICE LE 5 THEN OUTPUT P2PTOP5;
		RUN;
		...
		PROC PRINT DATA = P2PTOP5  (OBS=&PRINTOBS);
			...
			VAR &USCHAR &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM &CMCHAR
			    &USVCOM &USTCOM &CMVCOM DIFMER &DIFCHAR
			    NVMATCH LOTDIFF WNDORDER COSTDIFF CHOICE;
			TITLE3 'CONCORDANCE CHECK - TOP 5 POSSIBLE MATCHES FOR SAMPLE U.S. MODELS';
		RUN;

		PROC SORT DATA = CMMODELS;
			BY &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM;
		RUN;

		PROC SORT DATA = P2PMODS;
			BY &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM;
		RUN;

		DATA P2PMODS;
			MERGE P2PMODS (IN=A) CMMODELS (IN=B);
			BY &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM;
			IF A & B THEN OUTPUT P2PMODS;
		RUN;

		PROC SORT DATA = USMODELS;
			BY &USMANF &USPRIM USLOT &US_TIME_PERIOD &USMON &USCONNUM;
		RUN;
		...
		**----------------------------------------------------------**;
		...
		DATA COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CONCORD
			 ISMODELS (DROP=&TOTALCOST CHOICE)
			 CVMODELS (KEEP=&USMANF &USPRIM USLOT &US_TIME_PERIOD &USMON &USCONNUM
			                NVMATCH &TOTALCOST);
			MERGE USMODELS (IN=A) P2PMODS (IN=B);
			BY &USMANF &USPRIM USLOT &US_TIME_PERIOD &USMON &USCONNUM;
				IF NVMATCH = . THEN NVMATCH = 3;
				IF A THEN OUTPUT COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CONCORD;
			IF A & B THEN OUTPUT ISMODELS;
			IF A & NOT B THEN OUTPUT CVMODELS;
		RUN;
		...
		DATA _NULL_;
			SET MATCH;
			IF _N_ = 1;
				IF NOBS GT 0 THEN MATCH = "YES";
				ELSE IF NOBS = 0 THEN MATCH = "NO";
		...
		DATA _NULL_;
			SET CV;
			IF _N_ = 1;
				IF NOBS GT 0 THEN CVCALC = "YES";
				ELSE IF NOBS = 0 THEN CVCALC = "NO";
		...
		PROC PRINT DATA = COMPANY.&RESPONDENT._&SEGMENT._&STAGE._CONCORD (OBS=&PRINTOBS);
			...
			VAR &USMANF &USPRIM USLOT &USMON &USCONNUM &US_TIME_PERIOD &USCHAR
			    &CMMANF &CMPRIM CMLOT &CM_TIME_PERIOD &CMMON CMCONNUM &CMCHAR
			    &USVCOM &USTCOM &CMVCOM DIFMER &DIFCHAR
...
%MACRO US8_LOTADJ;
	...
	%IF %UPCASE(&LOT_ADJUST) = CM %THEN
	%DO;

		PROC SORT DATA = ISMODELS;
			BY USLOT CMLOT;
		RUN;

		PROC SORT DATA = COMPANY.&RESPONDENT._&SEGMENT._&STAGE._LOTADJ
		     OUT = CMLOTDATA (KEEP=USLOT CMLOT LOTADJ);
		BY USLOT CMLOT;
		RUN;

		PROC PRINT DATA = CMLOTDATA;
			TITLE3 'CM LOT ADJUSTMENT FACTOR';
		RUN;

		DATA ISMODELS;
			MERGE ISMODELS (IN=A) CMLOTDATA (IN=B);
			BY USLOT CMLOT;
			IF A;
			LENGTH LOTHDATA $3.;
			IF A & B
...
%MACRO US10_LOT_ADJUST_OFFSETS;
	...
	%DO;
		...
		PROC SORT DATA = ISMODELS (DROP=&USCHAR &CMCHAR &DIFCHAR);
			BY &USMANF &USPRIM USLOT &US_TIME_PERIOD &USMON &USCONNUM;
		RUN;
		...
		DATA NVIDSIM;
			...
			DO;
				...
				USECEPOFST = 'NO';
				%MULTIPLE_CURR; /* Convert CM values in non-CM currency into CM currency, if necessary */
				LOTADJMT   = 0;
				%P2P_ADJUSTMT_CEP;
				%P2P_ADJUSTMT_LOTADJ;
	 			INDDOL   = (CMICC + CMISELL) * &XRATE1; /* CM total indirects */
				COMMDOL  = CMCOMM * &XRATE1;    		/* CM commissions */
				ICOMMDOL = CMINDCOM * &XRATE1;  		/* CM surrogate commission */
...
%MACRO US11_CVSELL_OFFSETS;
	...
	%DO;
		...
		PROC PRINT DATA = CMCVSELL;
			TITLE3 'COMPARISON MARKET SELLING EXPENSE & PROFIT RATIOS FOR CV';
		RUN;

		PROC SORT DATA = CVMODELS;
			BY USLOT;
		RUN;
	...
	DATA NVCV;
		SET NVCV;
			...
			CVSELLPR = 'YES';
			DSELCV   = DSELCVR * &TOTALCOST;  /* CM direct selling, excluding CREDCV */
			ISELCV   = ISELCVR * &TOTALCOST;  /* CM indirect selling, excluding INVCARCV */
			COMMCV   = COMMCVR * &TOTALCOST;  /* CM commissions */
			ICOMMCV  = ICOMCVR * &TOTALCOST;  /* CM surrogate commission */
			INVCARCV = INVCVR  * &TOTALCOST;  /* CM inventory carrying exp */
			CVPROFIT = PRATECV * &TOTALCOST;  /* Amount of CV profit, in CM currency */
			...
			TOTCV    = &TOTALCOST + DSELCV + ISELCV + COMMCV + CVPROFIT;
...
**	there are P2P comparisons, P2P_VARS will be set to: CMNETPRI DIFMER	**;
...
%MACRO US12_COMBINE_P2P_CV;
	...
	%DO;
		...
		DATA  USSALES;
			SET NVIDSIM NVCV (DROP=DSELCVR ISELCVR COMMCVR ICOMCVR INVCVR CREDCVR &TOTALCOST);
		RUN;

		%LET P2P_VARS	=  CMNETPRI DIFMER LOTDIFF LOTADJMT; /* P2P vars required */
	...
	%DO;
		...
		%LET P2P_VARS =  CMNETPRI DIFMER LOTDIFF LOTADJMT;
		%LET CV_VARS = ;
	...
	%DO;
		...
		DATA USSALES;
			SET NVCV (DROP=DSELCVR ISELCVR COMMCVR ICOMCVR INVCVR CREDCVR &TOTALCOST);
		RUN;
...
%MACRO US13_COHENS_D_TEST;
	...
	**********************************************************************;
		...
		%IF %UPCASE(&USMANUF) EQ NA OR %UPCASE(&CMMANUF)= NO %THEN
		%DO;
			%MACRO DPMANF_RENAME;
			%MEND DPMANF_RENAME;
		...
		%ELSE %IF %UPCASE(&USMANUF) NE NA AND %UPCASE(&CMMANUF)=YES %THEN
		%DO;
			%MACRO DPMANF_RENAME;
				&USMANF =BASE_MANUF
			%MEND DPMANF_RENAME;
		...
		%IF %UPCASE(&USPRIME) EQ NA OR %UPCASE(&CMPRIME) = NO %THEN
		%DO;
		...
		%ELSE %IF %UPCASE(&USPRIME) NE NA AND %UPCASE(&CMPRIME) = YES %THEN
		%DO;
			%MACRO DPPRIME_RENAME;
				&USPRIM = BASE_PRIME
			%MEND DPPRIME_RENAME;
...
%MACRO US14_WT_AVG_DATA;
    ...
    PROC SORT DATA = USSALES
        (KEEP = &USMANF &USPRIM USLOT &USMON &US_TIME_PERIOD
                SALE_TYPE &USCONNUM &USCHAR NVMATCH &USQTY &EXRATE1
                &EXRATE2 USNETPRI USCOMM USCREDIT USDIRSELL USPACK
                USECEPOFST CEPOFFSET COMOFFSET INDDOL COMMDOL ICOMMDOL
                &P2P_VARS &CV_VARS &AR_VARS &DP_COUNT &COHENS_D_PASS)
         OUT = USNETPR;
...
%MACRO US15_RESULTS;
	...
	**--------------------------------------------------------------**;
		...
		PROC SORT DATA =  COMPANY.&RESPONDENT._&SEGMENT._&STAGE._&OUTDATA
			...
			(KEEP=	&IMPORTER &USQTY USNETPRI&SUFFIX USVALUE PCTMARG
				    EMARGIN UMARGIN &USMANF SALE_TYPE NVMATCH
