...
/*                                                          */
/*          GENERIC VERSION LAST UPDATED 09/10/2014         */
/*                                                          */
...
%LET SV_WORKSHEET   = <       >; /* (T) WORKSHEET TO CONVERT - OFTEN SHEET1   */
%LET SV_NAME_RANGE  = <       >; /* (T) COLUMN RANGE IN WORKSHEET LISTING SV  */
                                 /*     NAMES. USE THE CONVENTION R3:R42.     */
                                 /*     THE BEGINNING OF THE RANGE IS OFTEN   */
                                 /*     CELL R3.                              */
%LET SV_VALUE_RANGE = <       >; /* (T) COLUMN RANGE IN WORKSHEET LISTING SV  */
                                 /*     VALUES. USE THE CONVENTION T3:T42.    */
                                 /*     THE BEGINNING OF THE RANGE IS OFTEN   */
                                 /*     CELL T3.                              */
...
%MACRO CALCULATE_CASH_DEPOSIT;
	...
	%MACRO PRINT_CASH_DEPOSIT(OUTDATA,METHOD);
		...
		%DO;
			...
			%MACRO TOTDUMP_LABEL;
				 TOTDUMPING = 'TOTAL AMOUNT*OF DUMPING *=============';
			%MEND TOTDUMP_LABEL;
		%END;

		PROC PRINT DATA = ANSWER_&OUTDATA NOOBS SPLIT='*';
			VAR TOTSALES TOTVAL TOTQTY POSDUMPING NEGDUMPING TOTDUMPING MINMARG MAXMARG MARGVAL MARGQTY
			    PCTMARV PCTMARQ ;
			TITLE3 "BUILD UP TO ANTIDUMPING DUTY DEPOSIT RATE";
	...
	%DO;
		%PRINT_CASH_DEPOSIT(MIXEDMARG,MIXED)
	%END;
	...
	/*--------------------------------------------------------------*/
		...
		PROC PRINT DATA = ANSWER NOOBS SPLIT = '*';
			...
			%DO;
				...
				LABEL 	WTAVGPCT_STND = "STANDARD METHOD*AD VALOREM*CASH DEPOSIT RATE*(E/A x 100)*(percent)* *================="
						WTAVGPCT_MIXED = "MIXED ALTERNATIVE*METHOD*AD VALOREM*CASH DEPOSIT RATE*(percent)* *================="
						WTAVGPCT_ALT = "A-to-T ALTERNATIVE*METHOD*AD VALOREM*CASH DEPOSIT RATE*(E/A x 100)*(percent)* *==================";
			...
			%DO;
				...
				LABEL PER_UNIT_RATE_STND = "STANDARD METHOD*PER-UNIT*CASH DEPOSIT RATE*( E/B )* *================="
					PER_UNIT_RATE_MIXED = "MIXED ALTERNATIVE*METHOD*PER-UNIT*CASH DEPOSIT RATE*(percent)* *================="
					PER_UNIT_RATE_ALT = "A-to-T ALTERNATIVE*METHOD*PER-UNIT*CASH DEPOSIT RATE*(E/A x 100)*(percent)* *==================";
				FORMAT PER_UNIT_RATE_STND PER_UNIT_RATE_MIXED PER_UNIT_RATE_ALT UNIT_MARGIN.;
			%END;
			TITLE3 "ANTIDUMPING DUTY CASH DEPOSIT RATES";
			FOOTNOTE1 &FOOTNOTE1;
			FOOTNOTE2 &FOOTNOTE2;
			FOOTNOTE3 &FOOTNOTE3;
	...
	%MACRO PRINT_ASSESS(INDATA);
		...
		PROC PRINT DATA = ASSESS_&INDATA SPLIT='*' WIDTH = MINIMUM;
			...
			LABEL US_IMPORTER  = 'IMPORTER**========'
			      ...
			      FORMAT ITENTVAL ITOTQTY COMMA16.2  DMINPCT ASESRATE COMMA8.2 PERUNIT COMMA9.3;
